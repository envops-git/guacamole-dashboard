<script>
	...
	$: searchValue, filterUsers(searchValue);
...
<div class="w-full h-[calc(100vh-100px) flex flex-col items-center pt-6 px-4">
	<p class="font-semibold text-xl mb-5">Users</p>
	<div class="w-full h-fit mt-2 border-2 border-blue-900 rounded-md bg-gray-200">
      ...
      <div class='w-[120px] h-full p-2'>
        <button
        on:click={() => location.assign('/admin/users/newUser')}
        class='border-2 border-white rounded w-full h-full hover:bg-blue-700 duration-100'>
          <p class='text-white font-semibold text-sm'> New User </p>
        </button>
      </div>
			<div class="w-[calc(100%-120px)] flex justify-center items-center">
				<p class="font-semibold text-white pr-[120px]">Username</p>
			</div>
		</div>
		{#if loading}
		...
		{:else if users.length}
			{#each filteredUsers as username, i}
				<button
					on:click={() => location.assign('/admin/users/' + username)}
					class="w-full min-h-[40px] {i == filteredUsers.length - 1
						? ''
