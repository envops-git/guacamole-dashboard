<script>
	...
	function filterConnections(searchValue) {
		...
		filteredConnections = connections.filter((connection) => {
			return connection.name.toLowerCase().includes(searchValue.toLowerCase());
		});
	}
...
<div class="w-full h-[calc(100vh-100px) flex flex-col items-center pt-6 px-4">
	<p class="font-semibold text-xl mb-5">Connections</p>
	<div class="w-full h-fit mt-2 border-2 border-blue-900 rounded-md bg-gray-200">
      ...
      <div class='w-[150px] h-full p-2'>
        <button
        on:click={() => location.assign('/admin/connections/newConnection')}
        class='border-2 border-white rounded w-full h-full hover:bg-blue-700 duration-100'>
          <p class='text-white font-semibold text-sm whitespace-nowrap'>New Connection</p>
        </button>
      </div>
			<div class="w-[calc(100%-120px)] flex justify-center items-center">
				<p class="font-semibold text-white pr-[120px]">Connection Name</p>
			</div>
		</div>
		{#if loading}
		...
		{:else if connections.length}
			{#each filteredConnections as connection, i}
				<button
					on:click={() => location.assign('/admin/connections/' + connection.id)}
